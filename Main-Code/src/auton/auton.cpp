...
namespace Auton
{
    ...
    void A(std::string &position)
    {
        ...
        chassis.turnToPoint(-1000, 0, 1000, {
                                                .forwards = false,
                                            });
    ...
    void priority1Blue()
    {
        ...
        chassis.moveToPose(67, 0, 90, 1000, {.maxSpeed = 30});

        // Move to and get disk to to the left
        chassis.moveToPose(60, 0, 270, 1000, {.maxSpeed = 30});
        Intake::intake();

        // Move to left disk again
        chassis.moveToPose(48, 0, 270, 1000, {.maxSpeed = 30});
        Intake::intake();

        // Get G1B
        chassis.moveToPose(27, -21, 40, 1000);
        Clamp::close();

        // Get next disk
        chassis.moveToPose(25, -44, 180, 1000, {.maxSpeed = 50});
        Intake::intake();

        // Go to bottom right corner
        chassis.moveToPose(63, -65, 90, 1000);
        chassis.moveToPose(63, -65, 90, 1000, {.maxSpeed = 20});
        Intake::intake();
        Intake::intake();
